...
Note that, for the test-clips task (ClipsVal), the same images were presented three times to each participants. More precisely, in a given session during which such scenes were shown, three runs showing the same images were acquired. Moreover, the order of these images varied between each run.
